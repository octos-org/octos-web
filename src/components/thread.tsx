...
export function Thread() {
  ...
  return (
    <ThreadPrimitive.Root className="flex h-full flex-col min-h-0 bg-surface-dark">
      {hasMessages ? (
        ...
        <div className="flex flex-1 min-h-0 flex-col items-center justify-center px-6">
          <h1 className="mb-3 text-3xl font-light tracking-tight text-text-strong">What can I help with?</h1>
          <p className="mb-10 text-sm text-muted">Ask anything, attach files, or record a voice message.</p>
        </div>
      )}
  ...
  return (
    <MessagePrimitive.Root className="flex justify-end px-4 py-3">
      <div className="flex flex-col items-end max-w-[75%]">
        <div data-testid="user-message" className="rounded-2xl rounded-br-md bg-user-bubble px-5 py-3 text-sm leading-relaxed text-text">
          <MessagePrimitive.Content />
        </div>
        <div className="mt-1.5 px-1 text-[10px] text-muted/50 select-none">{time}</div>
      </div>
...
function AssistantMessage() {
  return (
    <MessagePrimitive.Root className="flex px-4 py-3">
      <div data-testid="assistant-message" className="max-w-[80%] rounded-2xl rounded-bl-md bg-assistant-bubble px-5 py-4 text-sm leading-relaxed text-text elevation-1 transition-all duration-300 ease-out">
...
function Composer() {
  ...
  return (
    ...
    >
      ...
      >
        {cmdFeedback && (
          <div data-testid="cmd-feedback" className="mb-3 rounded-xl bg-accent-container px-4 py-2.5 text-xs text-accent">
            {cmdFeedback}
          </div>
        )}
        ...
        <div className="flex flex-col rounded-2xl bg-surface-container p-2 elevation-1">
          ...
          <div className="flex items-end gap-1.5">
            <ComposerPrimitive.Input
              ...
              placeholder={pendingFiles.length > 0 ? `${pendingFiles.length} file(s) attached — add a message...` : "Send a message..."}
          ...
          <div className="flex items-center gap-0.5 px-1 pt-1">
            ...
            <button
              ...
              disabled={recording !== null}
              className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg ${
                cameraStream ? "bg-green-600 text-white hover:bg-green-700" : "text-muted hover:bg-surface-elevated hover:text-accent"
              } disabled:opacity-30`}
